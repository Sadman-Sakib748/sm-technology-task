...
export function AuthModal({ isOpen, onClose, initialMode = "login" }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Form submitted:", { mode, formData, rememberMe });
  };
      ...
      <DialogContent className="font-geist sm:max-w-md w-full mx-4 p-6 overflow-y-auto max-h-[90vh] bg-card border-0 shadow-2xl">
        <div className="pt-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Submit */}
            <Button type="submit" className="w-full h-10 bg-accent hover:bg-accent/90 text-accent-foreground font-semibold text-sm rounded-lg">
              {mode === "login" ? "Login" : "Sign Up"}
            </Button>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button type="button" variant="outline" className="h-10 border-border hover:bg-muted bg-transparent" onClick={() => handleSocialLogin("google")}>
                Google
              </Button>
              <Button type="button" variant="outline" className="h-10 border-border hover:bg-muted bg-transparent" onClick={() => handleSocialLogin("facebook")}>
                Facebook
              </Button>
            </div>
